...
class Perro(Animal):
    def sonido(self):
        return (color(f"guaw guaw", "blue"))

class Gato(Animal):
    def sonido(self):
        return (color(f"miau miau", "blue"))

class Perico(Animal):
    def sonido(self):
        return (color(f"rocky cotoroky", "blue"))

# funcion polimorfismo reciben un objeto 
def hacer_hablar(objeto):
    print(color(f"\n( {objeto.nombre} ) dice ( {objeto.sonido()} )", "green"))
